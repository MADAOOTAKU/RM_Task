...
void OLED_ShowNum(u8 x, u8 y, int32_t num, u8 len, u8 size1, u8 mode)
{
    ...
    if (num < 0)
    {
        ...
        // 防止 num == INT32_MIN 直接取反溢出，使用 64 位过渡
        unum = (uint32_t)(-(int64_t)num);
        // 根据字体移动 x
        x += (size1 == 8) ? 6 : (size1 / 2);
    }
    else
    ...
    // 按位显示（保留原逻辑: 不做前导空格抑制，0 仍显示 '0'）
    for (t = 0; t < len; t++)
    {
        temp = (unum / OLED_Pow(10, len - t - 1)) % 10;
        OLED_ShowChar(x + (size1 / 2 + m) * t, y, (u8)('0' + temp), size1, mode);
    }
}
