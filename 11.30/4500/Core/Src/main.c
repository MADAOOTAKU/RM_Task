...
int main(void)
{
  ...
  while (1)
  {
    ...
    // OLED_ShowNum(0, 48, Turn_PID_Flag, 1, 16, 1);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  //else if(count>18&&Huidu_Datas!=255)
    else if (Huidu_Datas != 255)
    {
      count = 0;
      Turn_PID_Flag = 0;
    }
  ...
  if(Turn_PID_Flag == 1)
  {
    ...
    //面对圆环，逆时针
    SET_MOTORS_SPEED(Huidu_Error + out +1000-1000, Huidu_Error - out + 1000+2000);
    count++;

    // 备用直角停车方案(顺时针右轮停)- out + 1000 + 2000
    // SET_MOTORS_SPEED(Huidu_Error- out + 1000 + 2000, Huidu_Error  + out + 1000-1000);

    //面对圆环，顺时针
    //SET_MOTORS_SPEED(Huidu_Error -out + 1000+4000, Huidu_Error+out + 1000);
